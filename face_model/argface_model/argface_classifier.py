...
class ArcFaceClassifier:
    ...
    def load_model(self):
        if not self.model_loaded:
            ...
            info(f"Model loaded: {model_save_path}")
        else:
    ...
    def print_evaluation_metrics(self, true_labels, predicted_labels, save_path=None):
        ...
        info(f"Accuracy Score: {acc_score:.4f}")
        info(f"Mean Absolute Error: {mae:.4f}")
        info(f"Mean Squared Error: {mse:.4f}")
        info(f"R2 Score: {r2:.4f}")
        ...
        if save_path:
            ...
            info(f'Confusion matrix saved to {os.path.join(save_path, "confusion_matrix.png")}')
